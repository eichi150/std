...
void Cmd_Ctrl::log(const std::string& msg){
	if(logger){
		logger->log(msg);
	}
}

void Cmd_Ctrl::log_input_process(int argc, const std::vector<std::string>& str_argv){
	//Log Output
	log(std::to_string(argc));
	std::string user_input_log;
	for(const auto& str : str_argv){
		user_input_log.append("[ " + str + " ] ");
	}
	log(user_input_log);
}